...
@Service
public class DiaProductsProcessor {
	...
	private static final Logger LOGGER=LoggerFactory.getLogger(DiaProductsProcessor.class);
	...
	public CompletableFuture<ResultMapper>  procesarProductos(String categoria) {
		...
		LOGGER.info(LibertadConstants.DIAONLINE_DESC + " Se obtuvieron "+todosLosProd.size()+" produtos con "+contadorLlamadas+" invocaciones al WS para la categoria "+categoria);
		try {
		...
		result.getMensajes().add(LibertadConstants.DIAONLINE_DESC + " Se procesaron correctamente los productos de la categoria "+categoria);
	...
	private void procesarProductosYGuardar(List<ProductMapper> todosLosProd, ResultMapper resultMapper) {
		...
		Map<String, DiaProduct> mapProductosDia = new HashMap<String, DiaProduct>();
		for (ProductMapper productVtex : todosLosProd) {
			DiaProduct product =  ProductBuilder.generateDiaProduct(productVtex);
		...
		productService.saveAll(new ArrayList<DiaProduct>(mapProductosDia.values()));
