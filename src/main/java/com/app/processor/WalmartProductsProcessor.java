...
@Service
public class WalmartProductsProcessor {

	@Value( "${walmart.base.url}" )
	...
	private static final Logger LOGGER=LoggerFactory.getLogger(WalmartProductsProcessor.class);
	...
	public CompletableFuture<ResultMapper>  procesarProductos(String categoria) {
		...
		LOGGER.info(LibertadConstants.WALMART_DESC + " Se obtuvieron "+todosLosProd.size()+" produtos con "+contadorLlamadas+" invocaciones al WS para la categoria "+categoria);
		try {
		...
		result.getMensajes().add(LibertadConstants.WALMART_DESC + " Se procesaron correctamente los productos de la categoria "+categoria);
	...
	private void procesarProductosYGuardar(List<ProductMapper> todosLosProd, ResultMapper resultMapper) {
		...
		Map<String, WalmartProduct> mapProductosDia = new HashMap<String, WalmartProduct>();
		for (ProductMapper productVtex : todosLosProd) {
			WalmartProduct product =  ProductBuilder.generateWalmartProduct(productVtex);
		...
		productService.saveAll(new ArrayList<WalmartProduct>(mapProductosDia.values()));
