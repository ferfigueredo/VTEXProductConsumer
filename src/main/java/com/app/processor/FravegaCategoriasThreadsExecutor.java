...
@Service
public class FravegaCategoriasThreadsExecutor {

	@Value( "${fravega.base.url}" )
	private String clientVtexUrl;
	
    @Autowired
	private FravegaProductsProcessor fravegaProductsProcessor;

	@Autowired
	private ProductProxy proxy;
	
	private static final Logger LOGGER=LoggerFactory.getLogger(FravegaCategoriasThreadsExecutor.class);
	...
	public ResultMapper procesarProductosFuture2() {
		...
		List<CompletableFuture<ResultMapper>> completableFutures =
				categoriasIds.stream().map(categoria -> fravegaProductsProcessor.procesarProductos(categoria)).collect(Collectors.toList());
