...
@Service
public class WalmartCategoriasThreadsExecutor {

	@Value( "${walmart.base.url}" )
	private String clientVtexUrl;
	
    @Autowired
	private WalmartProductsProcessor walmartProductsProcessor;

	@Autowired
	private ProductProxy proxy;
	
	private static final Logger LOGGER=LoggerFactory.getLogger(WalmartCategoriasThreadsExecutor.class);
	...
	public ResultMapper procesarProductosFuture2() {
		...
		List<CompletableFuture<ResultMapper>> completableFutures =
				categoriasIds.stream().map(categoria -> walmartProductsProcessor.procesarProductos(categoria)).collect(Collectors.toList());
